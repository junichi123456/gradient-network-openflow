...
資金を加える。**世界政府の残高（§17）が実際に使われる唯一の用途である**（固定額表で
運用するため実徴収額とは連動しないが、想定上の使途としては還付のみを認める）。
共和制における国家間競争がやや弱い、という課題認識から発案された
（現実にどのような利益をもたらすかは構想段階であり、還付額を含め運用しながら調整する）。

本書が扱うのは、この対抗戦を**既存の国家システムへ接続する統合レイヤー**である。

- 国家の参加資格判定（属国は宗主国に統合する）
- 開催条件の判定（初回90日後以降・前回開催から28日間隔・直近30日生産額上位4か国の自動選抜・
  上位1〜3位への33日ロックアウト。§6.4）
...
世界協議は**ミニゲームの範疇**であり、還付は経済の主軸ではない。実際の世界政府の徴収総額
（属国上納の世界政府取り分・援助金の償却・外交準備高の月次減価など）とは連動させず、
**固定額表**で運用する（調整のしやすさを優先）。財源はサーバー管理者の裁量により無制限で、
残高不足は発生しない（`minecraft_server_spec.md` §17）。
...
配分比は 10 : 6 : 3 : 1。**金額は暫定値。** 現実にどのような利益をもたらすかは構想段階であり、
運用しながら調整することを前提とする（`WorldCouncilPayout.AMOUNTS`）。同順位（共同優勝など）は、
その順位の額をそれぞれが満額受け取る。
...
### 6.4 開催条件（`core`実装、`plugin`配線は未着手・ユーザーへ確認して決定）

`minecraft_server_spec.md` §17「開催条件」の判定ロジックを `WorldCouncilSchedule`
（`core/src/main/java/jp/mcserver/core/worldcouncil/`、新規）に実装した。

| 項目 | 内容 |
|---|---|
| 初回 | サーバー開始から90日後以降（`earliestHostDay`） |
| 開催間隔 | 前回開催から28日経過後（同上）。日付条件は一度成立すると恒真（`dateReady`） |
| 選抜 | 参加資格（rank7以上）を満たしロックアウト中でない国家から、直近30日の生産額が
  高い順に4か国（`selectParticipants`）。4か国に満たなければ `Optional.empty()`（見送り） |
| ロックアウト | 開催した回の最終順位1〜3位に33日課す。4位は対象外
  （`withLocksAfterHosting`） |

**判定ロジックのみ core に実装し、plugin 側の自動配線は今回のスコープに含めない**
（ユーザーへ確認して決定）。理由は §3.4 と同じで、直近30日の生産額（§7.2）を国家ごとに
継続追跡する仕組みが `NationLedger` にまだ無く、`WorldCouncilRanking.rank` の呼び出し元
（BLOCK CONQUEST 側の進行）も無いため、自動選抜を実際に走らせる土台がまだ整っていない。
`/worldcouncil register`・`begin`・`finish` による手動運用は当面そのまま維持する。
土台が整った時点で、`WorldCouncilSchedule` の判定結果を `register` の自動発行に置き換える想定。
...
| `WorldCouncilSchedule` | 開催条件の判定（初回90日・28日間隔・直近30日生産額上位4か国の
  自動選抜・33日ロックアウト。§6.4） |

`CoreTests.worldCouncil()` で検証済み（33件）。合計 970 件、全件成功。
...
- ライブな国家ランク管理が無いため、`/worldcouncil register` のランクは管理者の申告に頼る
  （§3.4）
- 開催条件（初回90日・28日間隔・直近30日生産額上位4か国の自動選抜・33日ロックアウト）の
  判定ロジックは `core` に実装済みだが、`plugin` 側の自動配線（`NationLedger` の生産額継続
  追跡・`register` の自動発行）は未着手。現状は`/worldcouncil register`等の手動運用のまま
  （§6.4）
