 ...
 *
 * <p><b>Hub滞在中は判定窓を一時停止し、Hub退出時に続きから再開する</b>（§2.1）。
 * これは専用のAPIを持たない——呼び出し側が<b>Hub滞在中は {@link #advance} を
 * 一切呼ばない</b>だけで実現できる。窓の経過秒数・pitchの変化フラグはどちらも
 * 呼ばれなければ動かないため、Hubへ入った時点の状態がそのまま凍結し、退出後は
 * 同じ窓の続きとして再開する。日次上限（{@link #countedTodayMinutes}）も、
 * 呼ばれない間は消費されない。
 */
