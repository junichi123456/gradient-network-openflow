 ...
 *   <li><b>外交準備高</b> — 国民の稼得 exp の 0.03 倍が自動的に積み上がる。国内では流通できず、
...
public final class NationalAccounts {
    ...
    /**
     * 稼得 exp に対する外交準備高の計上率（§7.1）。
     *
     * <p>この係数は全国家に一律にかかるため、<b>値を変えても国内総生産ランキングの順位は
     * 変わらない</b>。決めているのは外交コストとの相対比だけである。0.5 では月次減価
     * （10%、月間計上Pに対し残高は9Pへ収束）で消える額が外交支出の総額を大きく上回り、
     * 準備高を使う理由が生まれなかった。rank25国家（27人、稼働率35%想定・月間稼得約
     * 8.5M exp）を例にすると、制裁1回（350,000 exp）が収束残高に占める割合は
     * 0.5で0.9%・0.05で9%・0.03で15%となる。0.03 はこの比率を実用的な水準に
     * 戻す値として選んだ（ユーザーへ確認して決定）。
     */
    public static final double ACCRUAL_RATE = 0.03;
