 ...
 *
 * <p>個体の召喚位置の足元Yを基準に、そこから14ブロック上（足元Y=1の会場ではY=15に相当）
 * から自由落下する浮遊剣。プレイヤーの真上を中心に半径3ブロック以内のどこかへ、
 * 重複しない位置で降り注ぐ。地面に刺さると0.5ブロック埋まり、高さ0.4ブロック・
 * 半径1ブロックの衝撃波を残す。
 */
public final class SwordRain {
     ...
     *
     * <p><b>絶対座標のYではなく、召喚位置基準。</b>足元Y=1の会場を基準に決めた値（Y=15）を
     * オフセットへ直した（15−1=14）。実際の高度は {@code 召喚位置の足元Y + この値}。
     */
    public static final double SPAWN_Y_OFFSET = 14.0;
