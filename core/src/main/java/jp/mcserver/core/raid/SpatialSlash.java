 ...
 * （銅の剣）が現れる。生成してから移動を始めるまでの待機は、{@link SpecialWaveTiming}
 * に従い生成ごとにランダム（実機の指摘で、毎回同じ拍子にならないよう固定値からランダムへ
 * 直した）。それまでは狙った相手の位置に召喚していたが、相手の足場（洞窟の中・段差の上など）
 * に引きずられて出現位置が安定しなかったため、個体自身の頭上という固定点に直した。
 *
