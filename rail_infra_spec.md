...
> まだ確認していない——`plugin` のビルドに使う Paper API が 1.26.2 以降の定数を持つように
> なってから確定する（§6。対象は1.26.3へ移行予定のため、この条件はすでに満たすはず）。
...
| 決定事項 | 選んだ方式 |
|---|---|
| 対象バージョン | **1.26.3 へ移行予定**（最新版に追従）。`plugin/build.gradle` の paper-api を差し替えた（実機ビルドは未確認） |
...
- **実機でのビルド・起動そのものが未確認。** paper-api 1.26.3・sqlite-jdbc 3.46.1.3 の
