...
- [ ] 攻撃モーションのあいだ以外も**動いている**（呼吸・首振り）
- [ ] 移動中は足が交互に動く。第二形態は**対角の足が同時に**出る
...
- [ ] 露出していないときの頭は倍率が乗らない
- [ ] 肩装甲を叩くと `受け止めた（破壊まで N）` が出る（**体力は減らない**）
- [ ] 装甲の残量が半分を切ると**橙に光る**
- [ ] 両肩装甲を壊すと `胴が弱点になった` が出て、以降の胴が `×1.8` になる
- [ ] 壊した装甲は**表示ごと消える**
...
| 動きがおかしい | まず `/raid info` で状態と tick を確認する（弱点の露出・激昂までの残り・装甲の残量まで出る） |
| 紙のアイテムが並ぶ | 見た目の指定が古い jar のまま。`git pull` してビルドし直す |
| 弱点に倍率が乗らない | 露出していない。パリイ・妨害・空振りのどれかを成立させる。激昂中も乗らない |
| 装甲を叩いても体力が減らない | 仕様どおり。装甲が受け止めた分は体力に入らない |
...
引数は順に **参加人数 / 1人あたりDPS / 被ダメ軽減% / パリイ成功率%** である。段階移行、被弾、装甲の破壊時刻、激昂の回数、倍率が乗ったダメージの割合、討伐時間、モーションの出現回数、通信量の見積りがその場で出る。**実機で見るべきは「見た目・当たり判定・体感」であり、数値はここで潰しておく。**
...
| 項目 | 状態 |
|---|---|
| 実装済み | 部位の生成と見た目、親子の変換合成、待機→移動→攻撃の周期、待機・歩行のループモーション、**状況に応じた技の選択**、ダメージ判定、ノックバック、妨害、パリイ、**弱点の露出と発光**、**装甲の破壊**、**激昂**、ボスバー、効果音とパーティクル、段階移行と骨格の入れ替え、除去 |
